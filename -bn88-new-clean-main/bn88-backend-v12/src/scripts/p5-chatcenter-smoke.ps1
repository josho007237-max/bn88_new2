...
if (-not $failed) {
  ...
  if (-not $listRes.ok) {
    ...
    if ($listRes.body.ok -eq $false) {
      Write-Step -Name 'GET /api/admin/chat/sessions' -StatusCode 200 -Ok $false -Detail 'ok=false'
      $failed = $true
      return
    }
    ...
    if (-not $items -or $items.Count -lt 1) {
      ...
      if (-not $listResRetry.ok) {
        ...
        if ($listResRetry.body.ok -eq $false) {
          Write-Step -Name 'GET /api/admin/chat/sessions (after seed)' -StatusCode 200 -Ok $false -Detail 'ok=false'
          $failed = $true
          return
        }
